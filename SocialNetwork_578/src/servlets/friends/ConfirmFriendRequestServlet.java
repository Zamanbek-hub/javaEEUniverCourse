...
public class ConfirmFriendRequestServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if(DBManager.confirmFriendRequest(friend_id, current_user.getId())){
                if(DBManager.deleteFriendRequest(friend_id, current_user.getId())) {
                    response.sendRedirect("/friends?success=true&type=2&search_user=" + search_user);
                } else {
                    throw new Exception();
                }
